  ...
  app.post(BASE_API_URL + "/traffic-injuries", (req, res) => {
    ...
    db.find({ auto_com: auto_com, year: year }, (error, docs) => {
      if (docs.length != 0) {
        ...
        !newTrafficInjury.auto_com ||
        !newTrafficInjury.year ||
        !newTrafficInjury.accident ||
        !newTrafficInjury.dead ||
        !newTrafficInjury.injure ||
        Object.keys(newTrafficInjury).length != 5
      ) {
  ...
  app.put(BASE_API_URL + "/traffic-injuries/:auto_com/:year", (req, res) => {
    ...
    db.find({ auto_com: auto_com, year: year }, (error, numReplaced) => {
      ...
      if (numReplaced.length == 0) {
        ...
        !updateTrafficInjury.auto_com ||
        !updateTrafficInjury.year ||
        !updateTrafficInjury.accident ||
        !updateTrafficInjury.dead ||
        !updateTrafficInjury.injure ||
        Object.keys(updateTrafficInjury).length != 5
      ) {
        res.sendStatus(400);
        console.log("\n400 - TRAFFIC INJURY WITH WRONG VALUES");
      } else if (
        updateTrafficInjury.auto_com != auto_com ||
        updateTrafficInjury.year != year
      ) {
        res.sendStatus(409);
        console.log("\n409 - AUTO_COM AND YEAR MUST BE SAME");
      } else {
        db.update(
          { auto_com: auto_com, year: year },
          { $set: updateTrafficInjury }
        );
        res.sendStatus(200);
        console.log(
